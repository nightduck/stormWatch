...
function getChart() {
    ...
    if (valFrom >= valTo) {
        ...
        $.get( "http://" + location.hostname + ":" + location.port + "/history/" + nodeNameCliked  + "/" + Math.round(valFrom.getTime()/1000 + now.getTimezoneOffset() * 60) + "/" + Math.round(valTo.getTime()/1000 + now.getTimezoneOffset() * 60), function( data ) {
            console.log(data);
